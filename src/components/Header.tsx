              ...
              {navigationItems.map((item) => (
                ...
                >
                  ...
                  {item.subItems && activeDropdown === item.label && (
                    <div className="absolute top-full left-0 w-56 bg-black/95 backdrop-blur-md border border-yellow-500/30 shadow-2xl shadow-yellow-500/10 rounded-lg overflow-hidden mt-2">
                      {item.subItems.map((subItem) => (
                        <Link
                          key={subItem.path}
                          to={subItem.path}
                          className="block px-6 py-3 text-gray-300 hover:text-yellow-500 hover:bg-yellow-500/10 transition-all duration-300 text-sm border-b border-yellow-500/10 last:border-0"
                        >
                          {subItem.label}
                        </Link>
                      ))}
